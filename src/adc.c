...
void init_dma() {
  ...
  //wraps around when buffer is full
  dma_channel_set_read_addr(dma_chan, &adc_hw->fifo, false);
  dma_channel_set_write_addr(dma_chan, adc_buffer, false);
  dma_channel_set_transfer_count(dma_chan, NUM_SAMPLES, true);
  dma_channel_set_chain_to(dma_chan, dma_chan);
...
void rf_init_i2c() {
    ...
    gpio_set_function(36, GPIO_FUNC_I2C); // SDA
    gpio_set_function(37, GPIO_FUNC_I2C); // SCL
